...
def csv_dict_reader(path,delimiter=',',code='utf-8'): #чтение из csv файлa
    with open(path, encoding=code) as file_obj:
        reader = list(csv.DictReader(file_obj, delimiter=delimiter))
    return reader

def writer_str_csv(data,path,type='w'): #запись в csv файл
...
#             if district =='нет':
#                 address['district']=DISTRICT[CITY_ENG[city]]
#             return address
#     return address
#
...
#         split_address=get_split_address(address_inner,url)
#     except:
#         address_inner='нет'
#         split_address={'region': 'нет',
#             'district': 'нет',
#             'city': 'нет',
#             'street': 'нет',
#             'number_of_home': 'нет'}
...
#     features['name_seller']=name_seller
#     features['link_seller']=link_seller
#     features['address_inner']=address_inner
#     features['secondary']=secondary
#     features.update(split_address)
#     return features
#
# def get_content (html,nead_features,page_count,pages_count):
#     soup = BeautifulSoup(html,'html.parser')
...
#         except:
#             online_view = 'Нет'
#         features['online_view']=online_view
#         try:
...
#         features['type_ad']=type_ad
#
#         features.update(get_inner_content(link))
#         features=filling_empty_features(features,nead_features)
#         writer_features_csv(features,nead_features)
#         # check_in_features(features.keys(),all_features)
#         time.sleep(1)
#     return
...
def writer_txt(feature,file_name,type='w'): #Запись в списка в текстовый файл
...
def unification_address_with_population(address):
    ...
    for vilage in population:
        ...
        address_vilage=vilage['Населёный пункт'].replace('г. ','').replace('г.','').replace('пгт ','').replace('село ','').replace('поселок ','').replace('посёлок ','').replace('железнодорожная станция ','').replace('железнодорожный блок-пост ','').replace('железнодорожный разъезд ','').replace('железнодорожный блок-пост ','').replace('населённый пункт Блок-Пост ','')
...
def get_split_address(address,url,show_d=False):
    ...
    try:
        for current_address in address:
            ...
            list_address['region'] = 'Амурская область'
            if 'область' in current_address:
                continue
            elif ('р-н' in current_address) or ('район' in current_address) or ('р-н' in current_address):
                continue
                if ('микрорайон' in current_address):
                    list_address['street'] = current_address
                    continue
                list_address['district'] = current_address
            elif current_address in DISTRICT.keys():
                continue
                list_address['district'] = DISTRICT[current_address]
                list_address['city'] = current_address
            elif current_address in G_O_:
                continue
                list_address['district'] = current_address
                list_address['city'] = G_O_[current_address]
            elif ('ул.' in current_address) or ('ш.' in current_address) or ('пр-т' in current_address) or ('шоссе' in current_address) or ('улица' in current_address) or ('ул ' in current_address) or (' ул' in current_address) or ('пер.' in current_address) or (' пер' in current_address) or ('б-р' in current_address) or (' лет ' in current_address):
                list_address['street'] = current_address
                continue
                ...
                list_address['number_of_home'] = current_address
                continue
            if show_d:
                print(current_address)
    except:
        print(address)
    if list_address['city'] == 'нет':
        list_address.update(get_split_address_from_url(url, address['district']))
    if list_address['district'] == 'нет':
        if list_address['city'] != 'нет':
            list_address.update(get_district_from_village(address['city']))
    return list_address
...
def parse(category):
    ...
    if html.status_code==200:
        ...
        for line in data:
            writer_txt(line['link'], str(category) + '.txt', 'a')
            # line.update(get_inner_content(line['link']))
    else:
        print('Не удалось получить html')

if __name__ == "__main__":
    while True:
        last_viewed=csv_dict_reader('last_viewed.csv')[0]
        for category in ALL_NEED_URL_FROM_CATEGORY.keys():
            print(category)
            if last_viewed[category]=='':
                html=get_html(ALL_NEED_URL_FROM_CATEGORY[category])
                pages_count=get_pages_count(html.text)
                time.sleep(1)
            parse(category)
